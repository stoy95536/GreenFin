...
export default function Documents() {
  ...
  const [selectedDomain, setSelectedDomain] = useState("CERTIFICATION");
  ...
  const loadDocuments = async () => {
    try {
      const data = await getFarmerDocuments(FARMER_ID);
  ...
  useEffect(() => { loadDocuments(); }, [currentFarmer.id]);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setMessage(null);
    try {
      const result = await uploadDocument(file, FARMER_ID, selectedDomain);
      setMessage(`上傳成功！OCR 擷取 ${result.fields.length} 個欄位`);
      await loadDocuments();
    } catch (err: any) {
  ...
  const handleViewFields = async (doc: any) => {
    setSelectedDoc(doc);
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <p className="text-xs text-gray-400 mt-2">支援: PDF, JPEG, PNG, XLSX (最大 10MB)</p>
      </div>

      {message && (
        <div className={`rounded-lg p-3 text-sm ${message.includes("失敗") || message.includes("錯誤") ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}>
          {message}
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {documents.map((doc: any) => {
              const status = statusLabel[doc.status] || { text: doc.status, color: "bg-gray-100" };
              return (
                <div key={doc.id} className="flex items-center justify-between border border-gray-100 rounded-lg p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{doc.domain} ｜ {doc.source_level}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {doc.status === "OCR_COMPLETED" && (
                      <button
                        ...
                        className="text-xs bg-greenfin-100 text-greenfin-700 px-2 py-1 rounded hover:bg-greenfin-200"
                      >
                        處理
                      </button>
                    )}
              ...
              <tbody>
                {fields.map((f: any) => (
                  <tr key={f.id} className="border-b border-gray-50">
                    ...
                    <td className="py-2">{f.raw_value}</td>
                    <td className="py-2 text-greenfin-700">{f.normalized_value || "—"}</td>
                    <td className="py-2">
                      <span className={f.confidence < 0.7 ? "text-orange-600" : "text-green-600"}>
